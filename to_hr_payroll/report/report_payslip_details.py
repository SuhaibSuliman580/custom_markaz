...
class PayslipDetailsReport(models.AbstractModel):
    _name = 'report.to_hr_payroll.report_payslipdetails'
    ...
    def get_lines_by_contribution_register(self, payslip_lines):
        ...
        for payslip_id, lines_dict in result.items():
            ...
            for register, lines in lines_dict.items():
                res[payslip_id].append({
                    'register_name': register.sudo().name,
                    'total': sum(lines.mapped('total')),
                })
                for line in lines:
                    res[payslip_id].append({
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'docs': payslips,
            'data': data,
            'get_details_by_rule_category': self.get_details_by_rule_category(payslips.mapped('details_by_salary_rule_category').filtered('appears_on_payslip')),
            'get_lines_by_contribution_register': self.get_lines_by_contribution_register(payslips.mapped('line_ids').filtered('appears_on_payslip')),
        }
