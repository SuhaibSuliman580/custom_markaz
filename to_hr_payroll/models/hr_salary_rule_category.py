...
class HrSalaryRuleCategory(models.Model):
    ...
    paid_by_company = fields.Boolean(string='Paid by Company', tracking=True,
                                     compute="_compute_paid_by_company", store=True, readonly=False,
    ...
    def _compute_display_name(self):
        for r in self:
            r.display_name = '%s (%s)' % (r.name, r.code)

    @api.depends('parent_id')
    def _compute_paid_by_company(self):
        for r in self:
            r.paid_by_company = r.parent_id.paid_by_company

    def _compute_salary_rules_count(self):
        rules_data = self.env['hr.salary.rule']._read_group([('category_id', 'in', self.ids)], ['category_id'], ['__count'])
        mapped_data = {category.id: count for category, count in rules_data}
        for r in self:
    ...
    def _sum_salary_rule(self, localdict, amount):
        ...
        localdict['categories'].dict[self.code] = localdict['categories'].dict[self.code] + amount if self.code in localdict['categories'].dict else amount
        return localdict

    def action_view_salary_rules(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('to_hr_payroll.action_salary_rule_form')
        ...
        if rules_count != 1:
            action['domain'] = "[('id', 'in', " + str(salary_rules.ids) + ")]"
        elif rules_count == 1:
            res = self.env.ref('to_hr_payroll.hr_salary_rule_form', False)
            action['views'] = [(res and res.id or False, 'form')]
            action['res_id'] = salary_rules.id
        return action
